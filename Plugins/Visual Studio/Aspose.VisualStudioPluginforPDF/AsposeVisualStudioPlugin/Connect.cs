...
using AsposeVisualStudioPluginPdf.GUI;
using AsposeVisualStudioPluginPdf.Properties;
using Microsoft.VisualStudio.CommandBars;

namespace AsposeVisualStudioPluginPdf
{
    public class Connect : Extensibility.IDTExtensibility2, IDTCommandTarget
    {
        ...
        private const string Pdf_COMMAND_NAME = "AsposePdfVSPlugin";
        private const string Pdf_COMMAND_CAPTION = "New Aspose.Pdf Example Project";
        private const string Pdf_COMMAND_TOOLTIP = "Download and run Aspose.Pdf for .NET API Examples";
            ...
            try
            {
                ...
                for (int iloop = 1; iloop <= toolsPopup.CommandBar.Controls.Count; iloop++)
                {
                    if (toolsPopup.CommandBar.Controls[iloop].Caption == ASPOSE_MENU_CAPTION)
                    {
                        ...
                        {
                            if (cmdbtn.Caption == Pdf_COMMAND_CAPTION)
                            {
                                oBtn = cmdbtn;
                            }
                        }
                ...
                if (oBtn == null)
                {
                    ...
                    Command myCommand = commands.AddNamedCommand2(addInInstance, Pdf_COMMAND_NAME,
                        Pdf_COMMAND_CAPTION, Pdf_COMMAND_TOOLTIP, false, Resources.pnglogosmall, ref contextGUIDS,
           ...
           ref object varOut, ref bool handled)
        {
            try
            {
                ...
                {
                    if (cmdName == addInInstance.ProgID + "." + Pdf_COMMAND_NAME)
                    {
